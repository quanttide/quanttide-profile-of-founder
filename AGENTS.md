...
learn/note 是学习笔记目录，记录个人知识体系的构建过程。
...
| 类型 | 说明 | 示例 |
|------|------|------|
| 主题笔记 | 单一主题的深度学习 | llm.md, infra.md |
| 对比笔记 | 两个事物的对比分析 | qtadmin_vs_qtcloud_think.md |
| 研究笔记 | 对某个范式或方法的研究 | agents范式研究.md |
| 目录 | 多个相关笔记的集合 | code/ |

### 添加新笔记

1. **主题笔记**：直接创建 `learn/note/主题.md`
2. **目录**：创建 `learn/note/目录名/` 目录，包含 `index.md` 和 `README.md`
3. **更新 index**：在 `learn/note/index.md` 的主题列表中添加链接

### 命名规则

- 使用英文或中文命名
- 使用下划线 `_` 分隔词汇
- 示例：`llm.md`、`qtadmin_vs_qtcloud_think.md`、`agents范式研究.md`
...
ROADMAP 不是事前使用的工具，而是事后总结逐渐发现的规律，用于校准长期方向。

### 核心原则

- **不预设目标**：ROADMAP 是归纳而非演绎
- **定期回顾**：每个版本发布后更新 ROADMAP
- **动态调整**：根据实践发现调整阶段定义
...
- **探索期**：当前工作是否揭示了碎片信息收集的有效方法？
- **验证期**：当前工作是否验证了跨档案协作的有效性？
- **正式期**：当前工作是否提升了信息质量或贡献者体验？
...
保留多样性和个人化作为智能体训练的原始素材是主要维护目标之一。
